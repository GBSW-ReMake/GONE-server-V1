...
/**
 * {@code POST /api/v1/outings/{code}/locations}의 인가(@PreAuthorize) 통합 테스트(#97).
 *
 * <p>{@code OutingDepartAuthorizationTest}와 같은 이유로, {@code @WebMvcTest} 슬라이스가 아니라
 * 실제 필터 체인(+ {@code @EnableMethodSecurity})을 통해 TEACHER 역할로는 이 엔드포인트에 접근할
 * 수 없는지 확인한다.
 */
@SpringBootTest
@AutoConfigureMockMvc
class OutingLocationPingAuthorizationTest {
  ...
  void returns401WithoutToken() throws Exception {
    mockMvc.perform(post("/api/v1/outings/ANYCODE0001/locations")
