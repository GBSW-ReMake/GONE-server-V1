...
/**
 * 단건 알림 읽음 처리의 실제 DB·보안 필터 통합 테스트.
 *
 * <p>서비스 단위 테스트만으로는 LAZY 수신자 조회와 트랜잭션 종료 시점의 변경 감지를 검증할 수
 * 없다. 이 테스트는 실제 사용자·알림을 저장한 뒤 HTTP 요청을 보내 {@code isRead} 변경과
 * 소유권 검증을 확인한다.
 */
@SpringBootTest
...
class NotificationReadIntegrationTest {
  ...
  void persistsReadStatusForOwner() throws Exception {
    owner = saveUser();
    notification = saveNotification(owner, false);
  ...
  void returns200ForAlreadyReadNotification() throws Exception {
    owner = saveUser();
    notification = saveNotification(owner, true);
