...
- **`execute()` 안에서 handler 실행과 실패 기록이 같은 트랜잭션을 공유하는 문제(코드 리뷰
  ...
  고친 뒤 검증한다.
- **`cancel()`과 `execute()` 사이에 원자적 보호가 없는 문제(CodeRabbit PR 리뷰 2회차, 보류
  확정 2026-09-01)**: `execute()`는 `PENDING` 확인 후 `handler.handle()`을 호출하는데, 그
  사이에 `cancel()`이 별도 트랜잭션에서 같은 행을 지우고 먼저 커밋하면, 이미 메모리에 읽어둔
  `task`로 `handler.handle()`이 그대로 실행될 수 있다 — 예를 들어 학생이 막 복귀 버튼을
  눌러 `OUTING_TIMEOUT`이 취소된 직후에 이미 폴링이 집어간 같은 task가 실행되면서 "복귀
  하셨나요?" 알림이 한 번 더 나갈 수 있다. `handler.handle()` 직전에 `UPDATE ... WHERE
  status='PENDING'` 같은 원자적 claim을 추가하면 막을 수 있지만, 위 트랜잭션 분리 항목과
  마찬가지로 실제 handler가 없어 재현이 불가능하고 `execute()`의 실행 경계를 다시 설계해야
  하는 같은 종류의 작업이라 **#99에서 위 트랜잭션 분리 작업과 함께 처리하기로 보류
  확정**(보스 확인).
