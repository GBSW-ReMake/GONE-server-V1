...
## 남은 절차
- 15단계: Postman 컬렉션에 4개 엔드포인트 반영
- 17단계(Notion 반영)는 관리자 전용 API라 프론트 소비자가 아직 없지만, 다른 이슈와 동일하게
  진행하기로 확정(보스 확인, 2026-09-01) — 프론트 소비자 유무와 무관하게 일반 절차를
  따른다. 시점도 기존 규칙대로 머지 확인 후.
