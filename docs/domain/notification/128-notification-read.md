...
| 알림 ID가 0 이하이거나 숫자 형식이 아님 | `400` | `COMMON_001` |
...
인증 정보가 없거나 유효하지 않은 경우(세 엔드포인트 공통):
...
인증 실패 응답 형식은 위 단건 읽음 처리의 공통 인증 실패 예시를 따른다.
...
인증 실패 응답 형식은 단건 읽음 처리의 공통 인증 실패 예시를 따른다.
