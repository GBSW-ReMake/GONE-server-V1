...
## 결정 필요
이 두 경로를 어떻게 검증할지 보스 판단이 필요하다.
1. 지금은 정적 검토 + 라이브 카운트 검증까지만으로 병합하고, 실제 5번째 PR이 쌓이는
   시점(자연 발생)에 처음으로 실제 동작을 확인한다 — 추가 비용 없음, 다만 첫 실동작에서
   문제가 생기면 그때 대응해야 한다.
2. 팀 Discord 채널에 미리 "테스트 알림이 하나 갈 수 있다"고 공지한 뒤, 임계치를 일시적으로
   1로 낮춘 테스트 커밋으로 실제 트리거해보고 다시 5로 되돌린다 — 실동작을 미리 확인할 수
   있지만 팀 채널에 실제 알림이 가고, 테스트용 실제 이슈가 하나 생겼다 닫히는 흔적이 남는다.
