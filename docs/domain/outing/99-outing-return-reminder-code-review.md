...
| F | E 검증용 통합 테스트의 고정 sleep이 락 미작동 시에도 통과할 수 있음(2026-09-04 재스캔, 신규) | 아니오 | **해결 완료 — 아래 참고** |

A/B/C/D/E/F 전부 해결 완료.

## F 반영 현황(2026-09-04) — 경합 테스트를 명시적 동기화로 교체

**문제**: E를 검증하려고 추가한 `OutingTimeoutReturnRaceIntegrationTest`가 `Thread.sleep(800)`
하나로 "B가 락 대기 중이다"를 가정했는데, CI 부하 등으로 B 스레드 자체가 800ms 안에
스케줄조차 안 되면 `releaseLock.countDown()` 이후에야 B의 조회가 실행된다 — 이 경우
`@Lock`이 아예 없어도(고쳐야 할 버그가 그대로 있어도) 이벤트 순서(`A-locked`,
`checkpoint`, `B-locked`)가 우연히 같게 나와 테스트가 거짓으로 통과할 수 있었다.

**해결**: `aboutToQuery`라는 `CountDownLatch`를 B의 쿼리 호출 **직전**에 내리도록 추가해,
B가 실제로 요청을 보내기 시작했음을 메인 스레드가 확인한 뒤에만 다음 단계로 진행하게
했다. 그리고 짧은 여유(300ms) 뒤 `waiter.isDone()`이 `false`임을 직접 단언한다 — 락이
걸리지 않으면 이 조회는 거의 즉시 끝나므로, 이 단언이 실패하면 곧바로 "락이 작동하지
않는다"는 신호가 된다. `./gradlew build` 전체(테스트+체크스타일 포함) 통과 확인.
