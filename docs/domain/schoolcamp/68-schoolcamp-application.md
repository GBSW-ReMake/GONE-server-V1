...
>
> **후속(2026-08-21)**: [#84](./84-schoolcamp-ghost-claim-recovery.md)에서 관리자 수동
> 조치 없이 다음 claim 시도 시점에 자동으로 재점유를 시도하도록 개선했다(유예시간 2분 +
> 활성 신청 없음 확인).
...
- **커넥션 풀 경합 대응 — claim을 별도 트랜잭션으로 분리(확정, 2026-08-19)**: 재학생 300명
  ...
  문장" 수준(수 ms)으로 줄어 이 문제가 크게 완화된다. 트레이드오프: "검증 실패 시 트랜잭션
  ...
  확인한다.
  - **정정(코드 리뷰(#68) 지적 사항, 2026-08-19)**: 위 "문제가 사라진다"는 정확하지 않다 —
    `applyToCamp`가 이미 열어둔 바깥 트랜잭션(`sessionRepository.findById`가 커넥션 1개를
    점유한 상태)에서 `claim`을 호출하면, `REQUIRES_NEW`는 바깥 트랜잭션을 "일시 중단"만 할
    뿐 그 트랜잭션이 쥔 커넥션을 반납하지 않는다. 그래서 claim의 `UPDATE` 한 문장이 실행되는
    그 찰나에는 이 요청 1건이 커넥션을 2개(바깥 트랜잭션용 1 + claim의 새 트랜잭션용 1)
    동시에 물게 된다 — 100명 동시 신청 시나리오를 그대로 대입하면 그 순간의 커넥션 수요가
    "요청 수"가 아니라 "요청 수 × 2"에 가까워질 수 있다는 뜻이다. 락 보유 시간을 수 ms로
    줄인 효과 자체는 그대로 유효해 문제가 크게 완화되는 것은 맞지만, "완전히 사라진다"는
    과장이었다. 이 잔여 리스크는 별도 구조 변경(claim 전 세션 조회를 트랜잭션 밖으로 분리)
    없이, 아래 HikariCP 풀 크기 상향과 함께 관찰 지표로만 남겨둔다 — 이 이슈 범위에서
    구조를 더 손대지는 않는다.
