...
## 아직 결정 안 된 것 (리뷰 필요)
- **카테고리 초기 시드 목록/점수(위 "도메인 모델" 표) — 실제 규정표와 전수 대조 완료, 일치
  확인됨.** `docs/벌점상점_규정표.md`와 대조해 항목·점수 모두 일치함을 확인했다. 시드값을
  그대로 사용한다.
- **카테고리 추가/삭제(관리자 전용) API/화면의 세부 설계** — 보류, 재검토 시점 미정. "DB
  테이블로 관리하고 나중에 관리자가 추가/삭제한다"는 방향은 확정됐지만, 실제 엔드포인트
  경로/권한/화면은 별도 웹 관리자 페이지 이슈에서 다룬다.
- **누적 벌점 임계치 값(가정: 10점)** — 보류, 재검토 시점 미정. 실제 운영 기준 확인 필요.
  상점에도 임계치가 필요한지도 미결.
- **벌점 임계치 알림 재발송 정책** — 보류, 재검토 시점 미정. "처음 초과하는 순간에만" 보낼지,
  추가 벌점마다 재발송할지는 실제 알림 인프라(#37) 설계 시점에 다시 정해야 한다.
- **이의신청(소명) 프로세스 — 범위에서 완전히 제외하기로 확정, 만들지 않음.** 별도 이슈로
  분리하지 않는다.
- **"전체 학생" 조회(`studentUserId` 생략)에 기본 기간 강제가 필요한지** — 보류, 재검토 시점
  미정. 데이터가 쌓인 뒤 성능이 문제 되면 재검토한다.
- **정정 이력(감사 로그)을 별도로 남길지** — 보류, 재검토 시점 미정. 지금은 `updated_at`만
  갱신되고 이전 값은 남지 않는다. 감사 요구가 커지면 후속 이슈로 고려한다.
- **푸시 알림(#37) 연동 시점** — 보류, 재검토 시점 미정. #37이 완료된 뒤에야 연동 가능하다.
